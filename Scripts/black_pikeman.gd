...
func _process(_delta: float) -> void:
	if is_front_side:
		$black_pikeman.animation = "front"
	else:
		$black_pikeman.animation = "back"
...
func updates_moves_to_not_include_white_pieces(incoming_array_to_check):
	...
	var valid_moves = []
	var active_pieces = get_tree().get_nodes_in_group("black_pieces") # gets all white piece objects including positions
	...
	valid_moves = incoming_array_to_check
	for move in unplaceable_locations: # this code checks the incoming array and compares it to the locations of the white pieces, if there is a white piece there it does not add it to the array
		if incoming_array_to_check.has(move):
...
func strike_moves():
 ...
 # declares strike positions
	...
	for i in strike_positions:
		...
		if coord_to_map.x > min_left and coord_to_map.x < min_right and coord_to_map.y > min_top and coord_to_map.y < min_bottom:
			check_array.append(i)
	strike_positions = check_array
	var new_array = [] # array of unmovable positions local to the pikeman
	var no_move_to = get_tree().get_nodes_in_group("black_pieces")
	for i in no_move_to:
		if i is Area2D:
